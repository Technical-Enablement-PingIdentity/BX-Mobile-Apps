...
{
    __block  PIDUserSelectionObject *internalSelection = selection;
    [PingID setAuthenticationUserSelection:selection completionBlock:^(NSError * _Nullable error)
     {
         if(!error)
         {
             if (internalSelection.action == PIDActionTypeDeny) {
                 UIAlertController *alertController = [UIAlertController  alertControllerWithTitle:NSLocalizedString(@"auth_denied", nil) message:nil  preferredStyle:UIAlertControllerStyleAlert];
