...
/// Migration is currently in progress - you cannot make another API call until it is completed.
  ErrorCodeMigrationAlreadyRunning = 10014,
/// The device does not need to be migrated because it is already paired.
  ErrorCodeMigrationNotNeeded = 10015,
};


/// Enum that represents the migration status returned from the SDK
typedef SWIFT_ENUM(NSInteger, MigrationStatus, open) {
/// There is no PingID data that has to be migrated.
  MigrationStatusNotNeeded = 10000,
/// Migration completed.
  MigrationStatusDone = 10001,
/// Migration failed.
  MigrationStatusFailed = 10002,
/// Migration failed due to server error, client can try again.
  MigrationStatusTemporaryFailed = 10003,
/// Migration in progress.
  MigrationStatusInProgress = 10004,
};
...
///
/// returns:
/// a <code>completionHandler</code> which contains: <code>status</code> - integer that reflects the migration status,
/// <code>PairingInfo</code> - an array of the permitted authentication methods - see description of pair method
/// and <code>error</code> in case of an error will return NSError.
+ (void)migrateFromPingIDWithCompletionHandler:(void (^ _Nonnull)(enum MigrationStatus, PairingInfo * _Nullable, NSError * _Nullable))completionHandler;
...
///
/// returns:
/// Returns all the users that are paired to the device.
/// For each user the response includes its user id, first name, last name and the device id.
