...
struct Alert{    
    ...
    static func approveDeny(viewController: UIViewController, title: String, completionHandler: @escaping (_ approved: Bool?) -> Void){
        DispatchQueue.main.async {
            let authAlert = UIAlertController(title: title, message: nil, preferredStyle: .alert)
